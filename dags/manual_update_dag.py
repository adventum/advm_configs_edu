...
t0 = BashOperator(
    task_id='manual_clear_fetch_folder',
    bash_command=f'(cd {work_directory};rm -rf fetch/*)',
    dag=dag
    )

t1 = BashOperator(
    task_id='manual_update_and_truncate_stagings',
    bash_command=f'(cd {work_directory};python3 commands_executors/advm_etl.py {project} -S -I -T)',
    dag=dag
    )

t2 = BashOperator(
    task_id='manual_update_and_truncate_raw',
    bash_command=f'(cd {work_directory};python3 commands_executors/advm_etl.py {project} -R -I -T)',
    dag=dag
    )

t3 = BashOperator(
    task_id='manual_update_and_truncate_business',
    bash_command=f'(cd {work_directory};python3 commands_executors/advm_etl.py {project} -B -I -T -r ".*PIT")',
    dag=dag
    )

t4 = BashOperator(
    task_id='manual_update_and_truncate_calc_full_union',
    bash_command=f'(cd {work_directory};python3 commands_executors/advm_etl.py {project} -B -I -T -r "Calc|Full|Union")',
    dag=dag
    )

t5 = BashOperator(
    task_id='manual_fix_missing_fullgavisitstat',
    bash_command=f'(cd {work_directory};python3 commands_executors/advm_etl.py {project} -B -I -T -r '
                 f'"CalcAdSourceClear|CalcAdSourceDirtyUnion|CalcAdSourceDistinct|CalcAdSourcesDirty|FullGaVisitStat")',
    dag=dag
    )

t6 = BashOperator(
    task_id='manual_update_and_truncate_graph',
    bash_command=f'(cd {work_directory};python3 commands_executors/advm_etl.py {project} -G -I -T)',
    dag=dag
    )
"""
t7 = BashOperator(
    task_id='manual_update_attribution',
...
"""
t9 = BashOperator(
    task_id='manual_update_and_truncate_stagings_for_click_fraud',
    bash_command=f"(cd  {work_directory};python3 commands_executors/advm_etl.py {project} -S -I -T -r 'Staging_.*_adcost' "
                 f"-s `date -d '-7day' +%Y-%m-%d` -e `date -d 'yesterday' +%Y-%m-%d`)",
    dag=dag
    )

t10 = BashOperator(
    task_id='manual_update_and_truncate_raw_2',
    bash_command=f'(cd {work_directory};python3 commands_executors/advm_etl.py {project} -R -I -T)',
    dag=dag
    )
